...
codes
	T - m_time (parameter: a number representing the m_time--measured in seconds--when the following codes apply)
	S - start event (parameter: a string representing the m_desc for the event that starts)
	E - end the event
	P - print to screen
	A - archive
*/
	...
	for (auto day = 1; day < argc; ++day)
	{
		...
		for (::g_sysClock = 0u; ::g_sysClock < secInDay; ++::g_sysClock)
		{
			...
			while (m_time == ::g_sysClock)
			{
				...
				switch (opcode)
				{
				case 'T': // a new m_time code, this is the exit from the while loop (back to for loop)
					in >> m_time;
					break;
